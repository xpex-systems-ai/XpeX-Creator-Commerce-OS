...
import { DeployCommandBox, DeployEnvironmentMatrix, DeployGuardrailList, DeployReadinessGroupPanel, MetricCard, OperationModeBadge, SafetyNotice, SectionHeader, XpeXStandaloneNotice, XpeXPageShell } from '../components';
...
export default function XpeXDeployReadinessPage() {
  ...
  return <XpeXPageShell eyebrow="Phase 16 · Standalone Open Operations" title="Deploy Readiness" description="Checklist visual para validar /xpex-commerce aberto por padrão, Vercel Preview e Railway backend/banco em modo seguro, sem secrets, sem APIs externas e sem automações perigosas.">
    <XpeXStandaloneNotice context="deploy-readiness" />
    ...
    <section className="rounded-[2rem] border border-[#22C55E]/25 bg-[#22C55E]/10 p-7"><SectionHeader eyebrow="Auth boundary" title="Corredor XpeX aberto, casa Postiz preservada" description="A Fase 16 abre /xpex-commerce e subrotas sem login por padrão, mas não torna públicas APIs ou rotas privadas do Postiz." /><div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><MetricCard label="/xpex-commerce" value="Open" detail="Superfície standalone/manual-first liberada por padrão." /><MetricCard label="Postiz privado" value="Protected" detail="/analytics, /settings, /launches e rotas privadas seguem exigindo auth." /><MetricCard label="/api" value="Not exposed" detail="Matcher continua excluindo APIs do bypass público." /><MetricCard label="Rollback" value="Env true" detail="XPEX_COMMERCE_REQUIRE_AUTH=true reativa a proteção Postiz para XpeX." /></div></section>
    ...
    <section className="rounded-[2rem] border border-amber-300/30 bg-amber-300/10 p-7 text-sm leading-7 text-amber-50"><strong>Escopo seguro da Fase 16:</strong> esta página prepara deploy e readiness operacional; ela não conecta Mercado Livre API, Dub, n8n, OpenAI, WhatsApp, anúncios pagos ou redirect público. Também não coleta secrets, não rastreia usuários silenciosamente, não adiciona cookies e não altera schema de banco.</section>
  </XpeXPageShell>;
}
