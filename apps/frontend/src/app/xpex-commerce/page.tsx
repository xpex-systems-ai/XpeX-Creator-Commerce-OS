...
export default function XpeXCommercePage() {
  ...
  return <XpeXPageShell eyebrow="Phase 10 · Operação Real Controlada" title="XpeX Creator Commerce OS" description="Máquina de vendas com produto piloto, criador Anderso, campanha Seu quarto vira palco, link manual, criativos e aprovação humana — preservando localStorage, backend opt-in, importação, CRUD controlado, fallback seguro e sem integrações externas.">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]"><article className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-7"><SectionHeader eyebrow="Agentes mockados" title="Orquestração visual sem APIs reais" description="Sem Mercado Livre, provedor de links, orquestração externa, banco, Prisma ou geração IA real nesta fase." /><div className="mt-6 grid gap-4 md:grid-cols-2">{data.agents.map((agent) => <article key={agent.name} className="rounded-2xl border border-white/10 bg-[#081322]/80 p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-bold text-white">{agent.name}</h3><StatusBadge>{agent.status}</StatusBadge></div><p className="mt-3 text-sm leading-6 text-white/58">{agent.role}</p></article>)}</div></article><aside className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-7"><SectionHeader eyebrow="Próximos passos" title="Fase 10 ativa" /><ul className="mt-5 space-y-3 text-white/70"><li>✓ Produto piloto projetor portátil</li><li>✓ Campanha Seu quarto vira palco</li><li>✓ Criador Anderso com revisão manual</li></ul></aside></section>
  </XpeXPageShell>;
}
