...
export default function XpeXCommercePage() {
  ...
  return <XpeXPageShell eyebrow="Phase 11 · Métricas e Performance" title="XpeX Creator Commerce OS" description="Máquina de vendas com produto piloto, criador Anderso, campanha Seu quarto vira palco, métricas manuais, tracking, leads, performance e calendário — preservando localStorage, backend opt-in, importação, CRUD controlado, fallback seguro e sem integrações externas.">
    ...
    <section className="rounded-[2rem] border border-[#8DB4FF]/25 bg-[#8DB4FF]/10 p-7"><SectionHeader eyebrow="Controle de Performance" title="Fase 12 ativa" description="Métricas manuais, links rastreáveis, UTMs, atribuição manual e calendário operacional sem integrações externas." /><div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4"><ModuleCard href="/xpex-commerce/metrics" title="Painel de Métricas" detail="Visualizações, cliques, leads, mensagens, vendas manuais, comissão e criativos vencedores." cta="Abrir métricas" /><ModuleCard href="/xpex-commerce/tracking" title="Rastreamento Manual" detail="Registrar por canal, criativo e data sem scraping ou rastreamento automático." cta="Registrar tracking" /><ModuleCard href="/xpex-commerce/performance" title="Performance do Piloto" detail="Funil e recomendações por regras locais, sem IA externa." cta="Ver performance" /><ModuleCard href="/xpex-commerce/calendar" title="Calendário" detail="Plano de 7 dias para Anderso com publicação manual." cta="Abrir calendário" /><ModuleCard href="/xpex-commerce/utm-builder" title="Construtor de UTMs" detail="Gerar URLs UTM localmente com bloqueio de protocolos inseguros." cta="Criar UTMs" /><ModuleCard href="/xpex-commerce/link-performance" title="Performance de Links" detail="Ranking por cliques, leads, vendas manuais e comissão estimada." cta="Ver links" /><ModuleCard href="/xpex-commerce/attribution" title="Atribuição Manual" detail="Associar leads e vendas manuais a links rastreáveis internos." cta="Atribuir" /></div></section>
    <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
      ...
      <ModuleCard href="/xpex-commerce/links" title="Links Rastreáveis" detail="Slugs, UTMs e métricas atribuídas manualmente sem redirect público real." cta="Ver links" />
    ...
    <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]"><article className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-7"><SectionHeader eyebrow="Agentes mockados" title="Orquestração visual sem APIs reais" description="Sem Mercado Livre, provedor de links, orquestração externa, banco, Prisma ou geração IA real nesta fase." /><div className="mt-6 grid gap-4 md:grid-cols-2">{data.agents.map((agent) => <article key={agent.name} className="rounded-2xl border border-white/10 bg-[#081322]/80 p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-bold text-white">{agent.name}</h3><StatusBadge>{agent.status}</StatusBadge></div><p className="mt-3 text-sm leading-6 text-white/58">{agent.role}</p></article>)}</div></article><aside className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-7"><SectionHeader eyebrow="Próximos passos" title="Fase 12 ativa" /><ul className="mt-5 space-y-3 text-white/70"><li>✓ Produto piloto projetor portátil</li><li>✓ Campanha Seu quarto vira palco</li><li>✓ Criador Anderso com revisão manual</li><li>✓ Métricas, tracking, leads e performance manuais</li></ul></aside></section>
  </XpeXPageShell>;
}
