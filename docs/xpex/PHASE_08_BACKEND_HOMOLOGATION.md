...
- The frontend backend client lives in `apps/frontend/src/app/xpex-commerce/lib/backend-client.ts` and uses relative authenticated requests under `/xpex-commerce` with `credentials: 'include'`.
...
Phase 08 adds `GET /xpex-commerce/diagnostics` inside the existing authenticated controller. It derives the organization from `GetOrgFromRequest()` and returns only safe technical status: `moduleEnabled`, `organizationScoped`, `prismaAvailable`, `timestamp`, and `supportedResources`.

The endpoint does not return users, leads, tokens, secrets, private URLs, credentials, or infrastructure details.
...
Opt-in mode. When `NEXT_PUBLIC_XPEX_COMMERCE_BACKEND_ENABLED=true`, the frontend calls the relative authenticated backend routes. Diagnostics are checked along with the existing resource reads.
